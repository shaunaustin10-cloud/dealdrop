      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-slate-800/50 p-4 rounded-xl border border-slate-700 h-full flex flex-col">
             <div className="flex justify-between items-start mb-3">
                <h4 className="text-slate-400 text-sm font-bold uppercase flex items-center gap-2">
                  <TrendingUp size={16} /> Market Validation
                </h4>
                {market && (
                    <div className="flex flex-col items-end">
                        <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${market.confidenceScore >= 80 ? 'bg-emerald-500/20 text-emerald-400' : market.confidenceScore >= 60 ? 'bg-amber-500/20 text-amber-400' : 'bg-rose-500/20 text-rose-400'}`}>
                ...
                <div className="space-y-4 flex-1 overflow-y-auto pr-1 max-h-[300px] custom-scrollbar">
                    ...
                    <div className="bg-slate-900/50 p-3 rounded-lg border border-slate-700/50">
                        <div className="flex justify-between items-center mb-1">
                            <span className="text-slate-400 text-xs uppercase font-bold">Verified ARV</span>
                    ...
                    <div className="bg-slate-900/50 p-3 rounded-lg border border-slate-700/50">
                        <div className="flex justify-between items-center mb-1">
                            <span className="text-slate-400 text-xs uppercase font-bold">Rent Est.</span>
                    ...
                    <div className="space-y-3">
                        {market.comps && market.comps.length > 0 && (
                            <div>
                                <h5 className="text-xs font-bold text-slate-500 uppercase mb-2">Recent Sales</h5>
                                    ...
                                    {market.comps.map((comp, i) => (
                                        <div key={i} className="flex justify-between items-center text-xs bg-slate-800/30 p-1.5 rounded border border-slate-700/30">
                                            <div className="flex flex-col truncate max-w-[65%]">
                                                <span className="text-slate-300 truncate" title={comp.address}>{comp.address}</span>
                                                <span className="text-[10px] text-slate-500">{comp.distance} • {comp.date ? comp.date.split('T')[0] : 'N/A'}</span>
                                            </div>
                                            <span className="font-mono text-emerald-400/90">${comp.price.toLocaleString()}</span>
                                        </div>
                                    ))}
                                </div>
                            </div>
                        ...
                        {market.rentComps && market.rentComps.length > 0 && (
                            <div>
                                <h5 className="text-xs font-bold text-slate-500 uppercase mb-2 border-t border-slate-700/50 pt-2">Rental Comps</h5>
                                <div className="space-y-1.5">
                                    {market.rentComps.map((comp, i) => (
                                        <div key={i} className="flex justify-between items-center text-xs bg-slate-800/30 p-1.5 rounded border border-slate-700/30">
                                            <div className="flex flex-col truncate max-w-[65%]">
                                                <span className="text-slate-300 truncate" title={comp.address}>{comp.address}</span>
                                                <span className="text-[10px] text-slate-500">{comp.distance} • {comp.daysOld ? `${comp.daysOld} days ago` : 'N/A'}</span>
                                            </div>
                                            <span className="font-mono text-blue-400/90">${comp.rent.toLocaleString()}</span>
                                        </div>
                                    ))}
                                </div>
                            </div>
