...
import { Trash2, ChevronRight, TrendingUp } from 'lucide-react';
...
const DealCard = ({ deal, onClick, onDelete }) => {
  ...
  return (
    <div 
      ...
      className="group bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 hover:border-emerald-500/50 rounded-2xl overflow-hidden transition-all duration-300 hover:shadow-2xl hover:shadow-emerald-900/20 flex flex-col h-full relative cursor-pointer"
    >
      {onDelete && (
        <button 
           ...
           <div className="flex items-center gap-3">
               ...
               <div className="flex flex-col">
                  ...
                  <div className="flex items-center gap-1 text-xs font-bold text-slate-700 dark:text-white">
                    {score > 75 ? <TrendingUp size={12} className="text-emerald-500 dark:text-emerald-400" /> : null}
                    {score}/100
                  </div>
               </div>
           </div>
