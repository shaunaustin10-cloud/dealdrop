...
import { LayoutGrid, Plus, Zap, User as UserIcon, Sun, Moon, ShieldAlert, Globe, Briefcase, Home, LogOut } from 'lucide-react';
...
import { FileWarning, X, Check, Loader2 as LoaderIcon } from 'lucide-react';

export default function MainApp() {
  const { user, logout } = useAuth();
  const { addDeal, updateDeal, deleteDeal } = useDeals();
  const { isPro, isVIP, loading: subLoading } = useSubscription();
  ...
  useEffect(() => {
    const checkAdmin = async () => {
      ...
      try {
        ...
        console.error("Admin check failed in MainApp", e);
        setIsAdmin(false);
      }
    };
    checkAdmin();
  ...
  const handleSelectDeal = (deal) => {
    // If it's a private pipeline deal, just select it
      ...
      <nav className="border-b border-slate-200 dark:border-slate-800 bg-white/50 dark:bg-midnight/80 backdrop-blur-md sticky top-0 z-40 transition-colors duration-300">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between">
          <Link to="/dashboard/pipeline" className="flex items-center gap-1.5 cursor-pointer flex-shrink-0" onClick={() => setSelectedDeal(null)}>
            ...
            <span className="font-serif text-lg md:text-xl text-slate-900 dark:text-white tracking-tight whitespace-nowrap">REI <span className="text-primary italic">Deal Drop</span></span>
          </Link>
          ...
          <div className="flex items-center gap-4">
            ...
            {user ? (
              <div className="flex items-center gap-2 md:gap-4">
                <Link to="/dashboard/profile" className="flex items-center gap-2 group p-1 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">
                  <div className="w-8 h-8 rounded-full bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 flex items-center justify-center overflow-hidden group-hover:border-emerald-500 transition-colors">
                  ...
                  <div className="hidden md:flex flex-col items-start leading-none">
                    <span className="text-[10px] uppercase font-bold text-slate-400 dark:text-slate-500 mb-0.5 tracking-tighter">Account</span>
                    <span className="text-sm font-bold text-slate-700 dark:text-slate-300 group-hover:text-primary transition-colors">{user.displayName || user.email?.split('@')[0] || 'Profile'}</span>
                  </div>
                </Link>
                <div className="h-8 w-px bg-slate-200 dark:bg-slate-800 hidden md:block"></div>
                <button 
                  onClick={handleLogout}
                  className="hidden md:block text-[10px] uppercase font-black tracking-widest px-3 py-1.5 rounded-lg border border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-400 hover:bg-red-50 hover:text-red-600 hover:border-red-200 dark:hover:bg-red-900/20 dark:hover:text-red-400 dark:hover:border-red-900/50 transition-all"
                >
                  Logout
                </button>
              </div>
                ...
                <div className="space-y-6">
                  ...
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
                      <div>
                        ...
                        <p className="text-slate-500 dark:text-slate-400 text-[10px] md:text-xs font-medium uppercase tracking-wider">
                          Personal Deal Flow & Private Analysis
                        </p>
                      </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl w-full max-w-lg relative z-10 overflow-hidden shadow-2xl animate-scale-in">
                ...
                <div className="p-6 bg-slate-50 dark:bg-slate-800/30 flex flex-col gap-3">
                    ...
                    >
                        {unlocking ? <LoaderIcon size={18} className="animate-spin" /> : <Check size={18} />}
