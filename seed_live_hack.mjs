...
async function seed() {
    ...
    try {
        cred = await signInWithEmailAndPassword(auth, testEmail, testPassword);
        console.log(`Logged in as existing user ${cred.user.uid}.`);
    } catch (e) {
    ...
    // Elevate privileges to admin using profile document
    console.log("Setting profile role to admin...");
    const profileRef = doc(db, 'artifacts', appId, 'profiles', cred.user.uid);
    await setDoc(profileRef, { role: 'admin', email: testEmail }, { merge: true });
    console.log("Profile updated!");
    
    const dealsRef = collection(db, 'artifacts', appId, 'users', cred.user.uid, 'deals');
    const publicDealsRef = collection(db, 'artifacts', appId, 'publicDeals');
    
    for (const deal of deals) {
        deal.sellerId = cred.user.uid;
        deal.publiclyVisible = (deal.adminVerificationStatus === 'approved');
        
        try {
            const dealDoc = await addDoc(dealsRef, deal);
            console.log(`Added user deal: ${deal.address}`);
            
            if (deal.adminVerificationStatus === 'approved') {
                 await setDoc(doc(publicDealsRef, dealDoc.id), {
                     ...deal,
                     originalId: dealDoc.id
                 });
                 console.log(`Added public deal: ${deal.address}`);
            }
        } catch(e) {
            console.error(`Failed to add deal ${deal.address}:`, e.message);
        }
    }
