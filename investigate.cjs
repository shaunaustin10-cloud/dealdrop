const admin = require('firebase-admin');
const sa = {
  "type": "service_account",
  "project_id": "web-app-30504",
  "private_key_id": "0116c1349e151a0b7a5130dbb40e87e28d4f6e73",
  "private_key": "-----BEGIN PRIVATE KEY-----\\\nKZ2MJZFVOBtUv4+86Ov9e9Gv\n-----END PRIVATE KEY-----\n",
  "client_email": "firebase-adminsdk-fbsvc@web-app-30504.iam.gserviceaccount.com",
  "client_id": "113421053664554934227"
};
admin.initializeApp({ credential: admin.credential.cert(sa), projectId: sa.project_id });
const db = admin.firestore();
const auth = admin.auth();
auth.getUserByEmail("test@gmail.com").then(user => {
  console.log("UID:", user.uid);
  ["default-app-id", "default-app-id", "web-app-30504"].forEach(id => {
    db.collection("artifacts").doc(id).collection("profiles").doc(user.uid).get().then(p => {
      if (p.exists()) console.log(id, "Profile:", p.data());
    });
    db.collection("artifacts").doc(id).collection("users").doc(user.uid).collection("deals").get().then(d => {
      if (deals.size > 0) console.log(id, "Deals:", d.size);
    });
  });
});
